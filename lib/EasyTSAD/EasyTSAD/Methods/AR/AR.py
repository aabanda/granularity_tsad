...
class AR(BaseMethod):
    ...
    def train_valid_phase(self, tsTrain: TSData):
        ...
        train_scores = torch.cat(scores, dim=0)
        train_scores = train_scores.numpy().flatten()

        y_hat_train = torch.cat(y_hat, dim=0)
        y_hat_train = y_hat_train.numpy().flatten()

        self.score_train = train_scores
    ...
    def test_phase(self, tsData: TSData):
        ...
        import pandas as pd
        # pd.DataFrame(y_hat).to_csv("/home/110646@TRI.LAN/PycharmProjects/tsad/Results/YHats/AR/185.csv", index=False)
