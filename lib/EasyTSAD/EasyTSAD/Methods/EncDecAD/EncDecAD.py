    ...
    def train_valid_phase(self, tsTrain: TSData):
        ...
        with torch.no_grad():
            for idx, (x, target) in loop:
                ...
                mse = torch.sub(output, target).pow(2)
                y_hats_train.append(output[:,-1].cpu())
                scores_train.append(mse[:,-1].cpu())
                loop.set_description(f'Testing: ')

        scores_train = torch.cat(scores_train, dim=0)
        # scores = 0.5 * (torch.log(self.sigma + self.eps) + (scores - self.mu)**2 / (self.sigma+self.eps))
        
        scores_train = scores_train.numpy().flatten()
        y_hats_train = torch.cat(y_hats_train, dim=0)
        y_hats_train = y_hats_train.numpy().flatten()

        self.score_train = scores_train
        self.y_hat_train = y_hats_train
