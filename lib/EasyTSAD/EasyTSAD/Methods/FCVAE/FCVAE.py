...
class FCVAE(BaseMethod):
    ...
    def train_valid_phase(self, tsTrain: TSData):
        ...
        with torch.no_grad():
            for i, (x, y_all, z_all) in loop:
                ...
                reconstructions.append(rec_x.squeeze(1).cpu())
        ...
        recon_windows = torch.cat(reconstructions, dim=0).numpy()  # shape: (num_windows, window_size)
        window_size = recon_windows.shape[1]
        num_windows = recon_windows.shape[0]
        original_length = tsTrain.train.shape[0]  # Use the true length

        sequence = np.zeros(original_length)
        counts = np.zeros(original_length)

        for i in range(num_windows):
            end = min(i + window_size, original_length)
            seq_slice = slice(i, end)
            win_slice = slice(0, end - i)
            sequence[seq_slice] += recon_windows[i, win_slice]
            counts[seq_slice] += 1

        counts[counts == 0] = 1  # avoid division by zero
        sequence = sequence / counts

        self.y_hat_train = sequence  
        self.score_train = scores  
    ...
    def test_phase(self, tsData: TSData):
        ...
        window_size = recon_windows.shape[1]
        num_windows = recon_windows.shape[0]
        original_length = tsData.test.shape[0]  # Use the true length
        ...
        self.reconstructed_sequence = sequence  # This is the full reconstructed series
    ...
    def get_reconstructed_series(self):
        """Returns the full reconstructed series after test_phase."""
        return getattr(self, 'reconstructed_series', None)   
