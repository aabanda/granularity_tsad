    ...
    def __init__(self, tsData: TSData, phase:str, window_size: int) -> None:
        ...
        if phase == "train":
            ...
            for i in range(self.sample_num):
                ...
                y_dim = torch.from_numpy(tsData.train_label[i : i + self.window_size]).size()
                if len(y_dim) > 1:
                    Y[i, :] = torch.from_numpy(tsData.train_label[i : i + self.window_size]).squeeze(1)
                else:
            ...
            for i in range(self.sample_num):
                ...
                y_dim = torch.from_numpy(tsData.valid_label[i : i + self.window_size]).size()
                if len(y_dim) > 1:
                    Y[i, :] = torch.from_numpy(tsData.valid_label[i : i + self.window_size]).squeeze(1)
                else:
            ...
            for i in range(self.sample_num):
                ...
                y_dim  = torch.from_numpy(tsData.test_label[i : i + self.window_size]).size()
                if len(y_dim) > 1:
                    Y[i, :] = torch.from_numpy(tsData.test_label[i : i + self.window_size]).squeeze(1)  
                else:
