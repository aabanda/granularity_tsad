...
╚══════╝╚═╝  ╚═╝╚══════╝   ╚═╝          ╚═╝   ╚══════╝╚═╝  ╚═╝╚═════╝ 
    ...
    def set_dataset(self, datasets, dirname=None, dataset_type="UTS", specify_curves=False, curve_names=None, train_proportion=None, valid_proportion=None):
        ...
        if specify_curves and isinstance(datasets, list) and len(datasets) > 1:
            raise TypeError("param datasets must be a string specifying ONLY ONE dataset when specify_curves is True.")
        if isinstance(datasets, list) and curve_names is None:
    ...
    def run_exps(self, method, training_schema, replace, cfg_path=None, diff_order=None, preprocess=None, hparams=None):
        """
        ...
        run_instance.do_exp(tsDatas=tsDatas, hparams=hparams, replace= replace)
    ...
    def do_evals(self, method, training_schema):
        ...
        for dataset_name, value in tsDatas.items():
            ...
            for score_file in score_files:
                ...
                normalized_array = (score - np.min(score)) / (np.max(score) - np.min(score))
                score = normalized_array * margins[1]
