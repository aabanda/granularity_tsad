...
class PathManager:
    ...
    def __init__(self, glo_cfg) -> None:                
        ...
        self.__train_score_dir = os.path.join(self.__res_p, self.glo_cfg["Results"]["train_score_dir"])
        self.__train_y_hat_dir = os.path.join(self.__res_p, self.glo_cfg["Results"]["train_y_hat_dir"])
    ...
    def get_yhat_path(self, method, schema, dataset, curve_name, build=True):
        path = os.path.join(self.__y_hat_dir, method, schema, dataset)
        if build is True:
            check_and_build(path)
        return os.path.join(path, "%s.npy"%curve_name)

    def get_train_score_path(self, method, schema, dataset, curve_name, build=True):
        path = os.path.join(self.__train_score_dir, method, schema, dataset)
        if build is True:
            check_and_build(path)
        return os.path.join(path, "%s.npy"%curve_name)
    
    def get_train_yhat_path(self, method, schema, dataset, curve_name):
        path = os.path.join(self.__train_y_hat_dir, method, schema, dataset)
        check_and_build(path)
        return os.path.join(path, "%s.npy"%curve_name)
