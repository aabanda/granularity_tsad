    ...
    def forward(self, x, time_emb=None):
        ...
        if exists(self.mlp) and exists(time_emb):
            ...
            try:
                ...
                reshaped_time_emb = time_emb.unsqueeze(1)
                time_emb = rearrange(reshaped_time_emb, "b c -> b c 1 1")
                scale_shift = time_emb.chunk(2, dim=0)

        # TODO: I have set this to NONE to make it work, is it ok?
        scale_shift = None
...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            size = init_size if ind == 0 else torch.Size(
                [dim_in, (last_size[1] + last_size[1] % 2) // 2, (last_size[2] + last_size[2] % 2) // 2])
            last_size = size
